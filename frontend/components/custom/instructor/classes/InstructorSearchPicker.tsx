...
export default function InstructorSearchPicker({
  ...
  // Emails already serving as instructor or co-instructor in this class
  const existingInstructorEmails = useMemo(
    () =>
      new Set(
        classUsers
          .filter(
  ...
  const classMemberEmails = useMemo(
    () => new Set(classUsers.map((u) => u.email.toLowerCase())),
    [classUsers],
  );
  ...
  const suggestions = useMemo(() => {
    ...
    return allUsers
      .filter((u) => {
        ...
        if (existingInstructorEmails.has(email)) return false;
      ...
      .slice(0, 8);
  }, [draft, allUsers, existingInstructorEmails, chipsSet]);
  ...
  const tryAddChip = (raw: string): boolean => {
    ...
    if (existingInstructorEmails.has(email)) {
      setError("This person is already an instructor in this class.");
      return false;
    }
  ...
  const handleInviteAll = async () => {
    ...
    for (const email of chips) {
      try {
        if (!classMemberEmails.has(email)) {
          await enrollUser(email, classID);
        }
        await promoteToCoInstructor(email);
        successCount++;
      } catch {
        failed.push(email);
      }
    ...
    if (successCount > 0) {
      toast.success(
        `Added ${successCount} co-instructor${successCount === 1 ? "" : "s"}.`,
      );
    }
    if (failed.length > 0) {
    ...
    <div className="space-y-3" ref={containerRef}>
      <div className="relative">
        ...
        >
          ...
          <Input
            ...
            placeholder={chips.length === 0 ? "Search name or email..." : ""}
      ...
      <div className="flex items-center justify-end gap-3">
        ...
        >
          {working
            ? "Adding..."
            : `Add ${chips.length || ""} Co-Instructor${chips.length === 1 ? "" : "s"}`.trim()}
        </Button>
      </div>
    </div>
  );
